...
ansible ~$PROJECT -m shell -a "sh /home/mobileweb/admin/checkmd5.sh $DATE $REGION $AREA $PROJECT $VERSION"
echo 'download over'
read

##backup and unzip
ansible ~$PROJECT -m shell -a "sh /home/mobileweb/admin/back2unzip.sh $DATE $REGION $AREA $PROJECT $VERSION $DIR $UPTYPE"
ansible ~$PROJECT -a "cat /home/mobileweb/project/$DIR/version.ini"
echo 'backup and unzip over'
read

##restart
echo "please run as root: ansible ~$PROJECT -m shell -a 'su - mobileweb -c \"sh /home/mobileweb/admin/restart_market.sh\"'"
##sudo ansible ~$PROJECT -m shell -a 'su - mobileweb -c "sh /home/mobileweb/admin/restart_market.sh"'
